...
const FullSearchBar = ({ input, setInput, checkIn, setCheckIn, checkOut, setCheckOut, guests, setGuests, handleClick }) => (
  <form className="flex items-center justify-between w-full h-full" onSubmit={handleClick}>
    ...
    <button
      type="submit"
      className="flex items-center justify-center w-12 h-12 bg-[#fc3c5c] text-white rounded-full mr-2"
    >
      <FiSearch size={24} />
    </button>
  </form>
...
const CompactSearchBar = ({ handleClick }) => (
  <div className="flex h-full w-full items-center justify-between px-4 py-2">
    ...
    <button
      type="button"
      className="flex items-center justify-center w-8 h-8 bg-[#fc3c5c] text-white rounded-full mr-[-10px]"
      onClick={handleClick}
    >
      <FiSearch size={15} />
    </button>
...
const SearchBar = () => {
  ...
  return (
    <div>
      <div
        onClick={handleFocus}
        className={`search-bar-container cursor-pointer mx-auto transition-all duration-300 ${
          isScrolled
